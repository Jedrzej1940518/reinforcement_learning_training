...
def translate_input(net_input):
    new_input = torch.tensor([net_input[0] / 800, net_input[1] / 50, net_input[2] / 20, net_input[3] / 100], dtype=torch.float) 
    return new_input #normalizing

def translate_output(net_output):
    actions = net_output.clamp(-1, 1)
    result = ((actions[0] > 0).item(), (actions[1] * 800).item(), (actions[2] * 800).item())
    return result

def train():
    env = gym.make(env_name, render_mode = "rgb_array" )
    ppo = SimplePPO(actor_net, a_space, critic_net, log_path, translate_input, translate_output, debug=True, debug_period = 30, target_device='cpu', minibatch_size=64, entropy_factor=0.00001, video_interval=30)
    ppo.train(env, 5000, export_model=True, resume=True)

def test():
    env = gym.make(env_name, render_mode = "human" )
    ppo = SimplePPO(actor_net, a_space, critic_net, log_path, translate_input, translate_output, debug=True, debug_period = 30, target_device='cpu', minibatch_size=64, entropy_factor=0.00001, video_interval=30)
    ppo.train(env, 5000, export_model=False, resume=True)

def write_baselines():
    env = gym.make(env_name, render_mode = "rgb_array")
    calculate_baselines(env, log_path, a_space, 30)
